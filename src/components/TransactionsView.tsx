...
export const TransactionsView: React.FC<TransactionsViewProps> = ({
  ...
  // Options for Account and Category Dropdowns
  ...
  // 2. Filter by search, account, category
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 sm:px-6">
                        <div className="flex items-center gap-2.5">
                          {tx.receipt === 1 && (
                            <span
                              className="text-violet-600 bg-violet-50 p-1 rounded-md"
                      ...
                      <td className="py-3.5 px-4">
                        <CustomSelect
                          ...
                          options={(state.settings?.categories || []).map((c) => ({
                            value: c,
                            label: c,
                          }))}
                          size="sm"
                        ...
                        <div className="flex flex-wrap items-center gap-1.5">
                          {tx.tags &&
                            tx.tags.map((tag) => (
                              <span
                                key={tag}
                                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-semibold bg-slate-100 text-slate-700 hover:bg-rose-50 hover:text-rose-700 transition cursor-pointer group/tag"
        ...
        <div className="px-6 py-3 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 flex items-center justify-between">
          <span>Showing {filteredTxs.length} transaction(s)</span>
          <span>
            Total Volume:{' '}
            <strong className="text-slate-900">
              {formatCurrency(filteredTxs.reduce((sum, t) => sum + t.amount, 0))}
            </strong>
          </span>
        </div>
      </div>
